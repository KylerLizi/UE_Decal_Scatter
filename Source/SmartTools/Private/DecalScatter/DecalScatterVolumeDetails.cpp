...
void FDecalScatterVolumeDetails::CustomizeDetails(IDetailLayoutBuilder& DetailBuilder)
{
	...
	DetailBuilder.HideProperty(GET_MEMBER_NAME_CHECKED(ADecalScatterVolume, MinDecalSpacing));
		...
		[
			...
			[
				...
				[
					...
					.Text(FText::FromString(TEXT("摆放贴花")))
				]
			]
		]
		+ SHorizontalBox::Slot()
	...
	TSharedRef<IPropertyHandle> MinDecalSpacingHandle = DetailBuilder.GetProperty(GET_MEMBER_NAME_CHECKED(ADecalScatterVolume, MinDecalSpacing));
	...
	MainCategory.AddProperty(MinDecalSpacingHandle);
